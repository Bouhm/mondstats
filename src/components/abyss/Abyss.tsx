...
function Abyss() {
  ...
  const [ filteredAbyssFloors, setFilteredAbyssFloors ] = useState<IAbyssBattle[]>(AbyssData)
  ...
  useEffect(() => {
    setFilteredAbyssFloors(_filterAbyss());
  }, [AbyssData, setFilteredAbyssFloors, characters, filters])
  ...
  const renderTopTeams = () => {
    const filteredTopTeams = _filterParties(abyssTopTeams);
    ...
    return (
      <>
        ...
        <div className="stage-half">
          <h2>{total} Teams</h2>
          {map(take(_filterParties(abyssTopTeams), stageLimitToggle["_ALL"] ? 10 : 5), ({party, count}, i) => (
